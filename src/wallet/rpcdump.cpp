...
UniValue importprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            "importprivkey \"privkey\" ( \"label\" rescan fStakingAddress )\n"
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid eXperience Points address or script");
    }
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by eXperience Points %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue bip38encrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("bip38encrypt", "\"PTwzSGpnSSRG5KX8sF9V8hP45Ej47bWS3p\" \"mypasphrase\"") +
            HelpExampleRpc("bip38encrypt", "\"PTwzSGpnSSRG5KX8sF9V8hP45Ej47bWS3p\" \"mypasphrase\""));
...
UniValue bip38decrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38decrypt \"address\" \"passphrase\"\n"
