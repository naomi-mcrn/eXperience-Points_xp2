    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zXP.";
            else if (!mint.isSeedCorrect)
                strReason = "The zXP seed used to mint this zXP is not the same as currently hold in the wallet";
            else
