...
UniValue submitbudget(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 7)
        throw std::runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"address\" monthly-payment \"fee-tx\"\n"
...
UniValue getbudgetprojection(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"TotalPayment\": xxx.xxx,      (numeric) Total payment amount in XP\n"
            "    \"MonthlyPayment\": xxx.xxx,    (numeric) Monthly payment amount in XP\n"
            ...
            "    \"Allotted\": xxx.xxx,           (numeric) Amount of XP allotted in current period\n"
            "    \"TotalBudgetAllotted\": xxx.xxx (numeric) Total XP allotted\n"
            "  }\n"
            "  ,...\n"
            "]\n"
