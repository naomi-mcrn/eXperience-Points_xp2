...
void CheckOffsetDisconnectedPeers(const CNetAddr& ip)
{
    ...
    if (setOffsetDisconnectedPeers.size() >= MAX_TIMEOFFSET_DISCONNECTIONS) {
        ...
        std::string strWarn2 = _("Please check that your computer's date and time are correct! If your clock is wrong eXperience Points Core will not work properly.");
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        std::string strDesc = "eXperience Points " + FormatFullVersion();
